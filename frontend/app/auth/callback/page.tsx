...
function AuthCallbackContent() {
  ...
  const handleCallback = async () => {
    ...
    const stateParam = searchParams.get('state'); // Base64 encoded state
    ...
    try {
      ...
      if (!storedState || !stateParam) {
        throw new Error('CSRF validation failed: missing state');
      }
      
      const decodedState = atob(stateParam);
      const parsedState = JSON.parse(decodedState);
      const storedParsed = JSON.parse(storedState);
      
      if (parsedState.csrf !== storedParsed.csrf) {
        throw new Error('CSRF validation failed: token mismatch');
      }
      
      deviceId = parsedState.deviceId || storedParsed.deviceId || 'unknown';
      
      // Clear the stored state
      sessionStorage.removeItem('oauth_state');
    } catch (csrfError) {
      console.error('CSRF validation error:', csrfError);
      // For backward compatibility, try to use state as device ID directly
      if (stateParam && !stateParam.startsWith('ey')) {
        deviceId = stateParam;
      }
    }
